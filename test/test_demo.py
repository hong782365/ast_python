...
async def receive_message(ws) -> TranslateResponseData:
    ...
    return TranslateResponseData(
        ...
        text=Response_data.text,  # Extract from actual response
    ...
    async def send_audio_chunks():
        try:
            for i, chunk in enumerate(audio_chunks):
                # if i >= 5:  # 从0开始计数，第5次是i=4
                #     logging.info("已达到最大发送次数(5)，停止发送")
                #     break
...
#             if resp.sequence == 3:
#                 logging.error(f"mock Session failed, message: {resp.message} logid: {log_id} event: {resp.event} message: {resp.message}")
#                 raise Exception("Session faild")
#                 break
            if resp.event == 153 or resp.event == 151:
...
# Example usage
async def main():
    conf = Config(ws_url=WS_URL,
                  app_key=APP_KEY,
                  access_key=ACCESS_KEY,
                  resource_id=RESOURCE_ID)
    start = time.time()
    # for _ in range(3):
    #     task_list = [asyncio.create_task(translate_v4(conf, "test_audio.wav", i)) for i in range(1, 61)]
    #     for task in task_list:
    #         await task
    # task = asyncio.create_task(translate_v4(conf, "test_audio.wav", 1))
    task = asyncio.create_task(translate_v4(conf, "/Users/bytedance/code/50s_no_data.wav", 1))
    await  task
    end = time.time()
